...
func NewGatewayServer(server *server.GrpcServer) *GatewayServer {
	...
	err := notification.RegisterNotificationServiceHandlerServer(context.Background(), httpMux, server.ServicesServer)
	if err != nil {
	...
	corsWrapper := cors.New(cors.Options{
		AllowedOrigins: []string{"http://localhost:8031", "http://localhost:8030"},
