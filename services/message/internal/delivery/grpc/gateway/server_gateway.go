...
func NewGatewayServer(server *server.GrpcServer) *GatewayServer {
	...
	err := message.RegisterMessageServiceHandlerServer(context.Background(), httpMux, server.ServicesServer)
	if err != nil {
	...
	corsWrapper := cors.New(cors.Options{
		AllowedOrigins: []string{"http://localhost:8021", "http://localhost:8020"},
